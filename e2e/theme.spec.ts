...
import { assignThemeSlotsDirectly, getFirstCustomThemeId, resetCustomThemes, setUserTier } from "./fixtures/db";
import { t } from "./fixtures/i18n";
import { testUsers } from "./fixtures/test-users";

// Serial mode: each test depends on state from the previous test (theme CRUD lifecycle).
test.describe("Theme Studio", () => {
  ...
  test("free user can create a theme with color pickers, Pro features are disabled", async ({ freeUser: page }) => {
    //* Arrange — clean up any themes from previous serial retries (freeUser fixture uses admin)
    ...
    await page.getByText("Page").first().waitFor();
    await page.getByText("Avatar").first().waitFor();
    await page.getByRole("button", { name: t.saveTheme }).click();

    //* Assert — theme saved (check toast first; it auto-dismisses), Pro inputs disabled
  ...
  test("pro user creates a theme with a custom color, applies it, and the color renders on the public page", async ({
    ...
    //* Arrange — clean up any themes from previous serial retries, then create a theme
    ...
    //* Act — save the theme, then assign it to both slots via direct DB update
    await page.getByRole("button", { name: t.saveTheme }).click();
    await page.getByText(t.themeCreated).waitFor();
    await page.waitForURL(/\/theme\/studio\/[a-f0-9-]+/);
    const themeId = await getFirstCustomThemeId(testUsers.pro.username);
    if (themeId == null) {
      throw new Error("Custom theme was not found in the database after save");
    }
    await assignThemeSlotsDirectly(testUsers.pro.username, themeId);

    //* Assert — custom theme renders on public page with correct color variable
    const publicUrl = `/${testUsers.pro.username}`;
  ...
  test("disabling dark mode hides dark picker and forces light on public page", async ({ proUser: page }) => {
    ...
    //* Assert — dark theme picker hidden (only the Dashboard card's "Dark theme" heading remains)
    await expect(page.getByText(t.darkTheme, { exact: true })).toHaveCount(1);
  ...
  test("downgraded user's custom theme still renders on public page but studio locks Pro fields", async ({
    ...
    //* Arrange — create a theme with a custom font as pro, assign it via DB, then downgrade
    await page.goto("/dashboard/theme/studio/new");
    await page.waitForLoadState("domcontentloaded");
    ...
    await page.getByText(t.themeCreated).waitFor();
    const editUrl = page.url();
    const themeIdMatch = editUrl.match(/studio\/([a-f0-9-]+)/);
    if (themeIdMatch == null) {
      throw new Error("Could not extract theme ID from URL: " + editUrl);
    }
  ...
  test("deleting an active theme resets slot to default preset", async ({ proUser: page }) => {
    //* Arrange — reset to a single active theme so deleting it resets the slot
    await resetCustomThemes(testUsers.pro.username);
    await page.goto("/dashboard/theme/studio/new");
    await page.waitForLoadState("domcontentloaded");
    await page.getByRole("button", { name: t.saveTheme }).click();
